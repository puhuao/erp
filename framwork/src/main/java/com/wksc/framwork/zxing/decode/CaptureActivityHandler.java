...
import com.wksc.framwork.zxing.view.ViewfinderResultPointCallback;
...
public final class CaptureActivityHandler extends Handler {
	...
	public void handleMessage(Message message) {
		switch (message.what) {
		case ZxingConfig.Zxing_Restart_Decode: // 准备进行下一次扫描
			Log.d(TAG, "Got restart preview message");
			restartPreviewAndDecode();
			break;
		case ZxingConfig.Zxing_Decode_Succeeded:
			...
			break;
		case ZxingConfig.Zxing_Decode_Failed:
			// We're decoding as fast as possible, so when one decode fails,
			// start another.
			state = State.PREVIEW;
			cameraManager.requestPreviewFrame(decodeThread.getHandler(),
					ZxingConfig.Zxing_Start_Decode);
			break;
		case ZxingConfig.Zxing_Decode_Result:
			Log.d(TAG, "Got return scan result message");
			activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
			activity.finish();
			break;
		case ZxingConfig.Zxing_Open_Result:
	...
	public void quitSynchronously() {
		...
		Message quit = Message.obtain(decodeThread.getHandler(), ZxingConfig.Zxing_Quit);
		quit.sendToTarget();
		...
		removeMessages(ZxingConfig.Zxing_Decode_Succeeded);
		removeMessages(ZxingConfig.Zxing_Decode_Failed);
	}
