...
final class DecodeHandler extends Handler {
	...
	public void handleMessage(Message message) {
		...
		switch (message.what) {
		case ZxingConfig.Zxing_Start_Decode:
			decode((byte[]) message.obj, message.arg1, message.arg2);
			break;
		case ZxingConfig.Zxing_Quit:
			running = false;
		...
		if (rawResult != null) {
			...
			if (handler != null) {
				Message message = Message.obtain(handler,
						ZxingConfig.Zxing_Decode_Succeeded, rawResult);
			...
			if (handler != null) {
				Message message = Message.obtain(handler, ZxingConfig.Zxing_Decode_Failed);
				message.sendToTarget();
			}
		}
	}
